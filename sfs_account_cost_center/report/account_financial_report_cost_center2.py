...
class account_balance(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            'time': time,
            'lines': self.lines,
            #'lines_by_cost_center': self.lines_by_cost_center,
    ...
    def _get_analytic_ledger(self, account, ctx={}):
        ...
        if account['type'] in ('other','liquidity','receivable','payable'):
            ...
            # TANTUMS cesar Equihua
            # obtenere centros de costos
            ...
            #where = """where aml.period_id in (%s) and aa.id = %s and aml.state <> 'draft' and aml.journal_id in (%s) and cc.id in (%s)"""%(periods,account['id'], journal_ids, cost_center_ids)
            ...
            #     inner join account_cost_center cc on cc.id = aml.centro_costo_id """ + where +\
            #     """ order by date, am.name"""
    ...
    def lines(self, form, level=0):
        ...
        def get_total_credit_or_debit_account_move_lines(account_id, cost_center_ids, credit_or_debit, period_ids):
            # credit_or_debit
            # 1 Credit
            # 2 Debit
            #print period_ids
            #print 'get_total_credit_or_debit_account_move_lines'
            #pdb.set_trace()
            amount_total = 0
            account_move_line_ids = self.pool.get('account.move.line').search(self.cr, self.uid, [('centro_costo_id', 'in', cost_center_ids),('account_id', '=', account_id),('period_id', 'in', period_ids)])
            if account_move_line_ids:
                account_move_line = self.pool.get('account.move.line').browse(self.cr, self.uid, account_move_line_ids)
                for acc_move_line in account_move_line:
                    if credit_or_debit == 1:
                        #print 'CUENTA ' + str(acc_move_line.account_id.name)
                        #print 'CREDITO ' + str(acc_move_line.credit)
                        #print 'DEBITO ' + str(acc_move_line.debit)
                        amount_total = amount_total + acc_move_line.credit
                    else:
                        amount_total = amount_total + acc_move_line.debit
            return amount_total
        ...
        cost_center_ids = []
        #period_ids_by_cost_center = []
        if form.get('cost_center_ids', []):
            cost_center_ids = form['cost_center_ids']
            #period_ids_by_cost_center = form['periods']
        ...
        total_init_activo = 0.0
        total_init_pasivo = 0.0
        total_init_capital = 0.0
        total_init_ingreso = 0.0
        total_init_gastos_vtas = 0.0
        total_init_gastos_admon = 0.0
        total_init_gastos_finan = 0.0
        total_init_gastos_amort = 0.0
        total_init_gastos = 0.0
        total_init_costos = 0.0

        total_debit_activo = 0.0
        total_debit_pasivo = 0.0
        total_debit_capital = 0.0
        total_debit_ingreso = 0.0
        total_debit_gastos_vtas = 0.0
        total_debit_gastos_admon = 0.0
        total_debit_gastos_finan = 0.0
        total_debit_gastos_amort = 0.0
        total_debit_costos = 0.0

        total_credit_activo = 0.0
        total_credit_pasivo = 0.0
        total_credit_capital = 0.0
        total_credit_ingreso = 0.0
        total_credit_gastos_vtas = 0.0
        total_credit_gastos_admon = 0.0
        total_credit_gastos_finan = 0.0
        total_credit_gastos_amort = 0.0
        total_credit_costos = 0.0

        total_ytd_activo = 0.0
        total_ytd_pasivo = 0.0
        total_ytd_capital = 0.0
        total_ytd_ingreso = 0.0
        total_ytd_gastos_vtas = 0.0
        total_ytd_gastos_admon = 0.0
        total_ytd_gastos_finan = 0.0
        total_ytd_gastos_amort = 0.0
        total_ytd_costos = 0.0

        total_balance_activo = 0.0
        total_balance_pasivo = 0.0
        total_balance_capital = 0.0
        total_balance_ingreso = 0.0
        total_balance_gastos_vtas = 0.0
        total_balance_gastos_admon = 0.0
        total_balance_gastos_finan = 0.0
        total_balance_gastos_amort = 0.0
        total_balance_costos = 0.0

        total_init_gastos = 0.0
        total_debit_gastos  = 0.0
        total_credit_gastos  = 0.0
        total_ytd_gastos  = 0.0
        total_balance_gastos  = 0.0
        ...
        for aa_id in account_ids:
            ...
            if not form['display_account_level'] or aa_id[3].level <= form['display_account_level']:
                ...
                #print str(res['id']) + str(res['type']) + str(res['code']) + '\t' + str(res['name']) + '\t' + str(res['parent_id']) + '\t' + str(res['level']) + '\t' + str(res['label']) + str(res['total']) + str(res['change_sign']) 
                ...
                if form['columns'] == 'qtr':
                    ...
                    for p_id in p:
                        ...
                        #print ctx_end['periods']
                        if form['inf_type'] == 'IS':
                            if cost_center_ids and aa_id[3].level != 0 and aa_id[3].type != 'view':
                                total_credit = get_total_credit_or_debit_account_move_lines(id, cost_center_ids, 1, ctx_end['periods'])
                                total_debit = get_total_credit_or_debit_account_move_lines(id, cost_center_ids, 2, ctx_end['periods'])

                                aa_brw_end.credit = total_credit
                                aa_brw_end.debit = total_debit
                                ...
                                # b = abs(b)
                            res.update({
                                'dbr%s'%pn: abs(self.exchange(d)),
                                'cdr%s'%pn: abs(self.exchange(c)),
                                'bal%s'%pn: abs(self.exchange(b)),
                            })
                        else:
                            if cost_center_ids and aa_id[3].level != 0 and aa_id[3].type != 'view':
                                total_credit = get_total_credit_or_debit_account_move_lines(id, cost_center_ids, 1, ctx_end['periods'])
                    ...
                    if form['inf_type'] == 'IS':
                        if cost_center_ids and aa_id[3].level != 0 and aa_id[3].type != 'view':
                            ...
                            # b = abs(b)
                        res.update({
                            'dbr5': abs(self.exchange(d)),
                            'cdr5': abs(self.exchange(c)),
                            'bal5': abs(self.exchange(b)),
                        })
                    else:
                        if cost_center_ids and aa_id[3].level != 0 and aa_id[3].type != 'view':
                            total_credit = get_total_credit_or_debit_account_move_lines(id, cost_center_ids, 1, ctx_end['periods'])
                    ...
                    if form['inf_type'] == 'IS':
                        if cost_center_ids and aa_id[3].level != 0 and aa_id[3].type != 'view': 
                            ...
                            # b = abs(b)
                        res.update({
                            'dbr13': abs(self.exchange(d)),
                            'cdr13': abs(self.exchange(c)),
                            'bal13': abs(self.exchange(b)),
                        })
                    else:
                        i,d,c = map(z,[aa_brw_init.balance,aa_brw_end.debit,aa_brw_end.credit])
                ...
                if to_include and form['analytic_ledger'] and form['columns']=='four' and form['inf_type'] == 'BS' and res['type'] in ('other','liquidity','receivable','payable'):
                    ...
                    res['mayor'] = []
                    #to_include = True
                if aa_id[3].account_category == 'result':
                    to_include = True

                if to_include:
                    if res['account_category'] == 'income':
                        total_init_ingreso = abs(res['balanceinit'])
                        total_debit_ingreso = abs(res['debit'])
                        total_credit_ingreso = abs(res['credit'])
                        total_ytd_ingreso = abs(res['ytd'])
                        total_balance_ingreso = abs(res['balance'])
                    elif res['account_category'] == 'cost':
                        total_init_costos = abs(res['balanceinit'])
                        total_debit_costos = abs(res['debit'])
                        total_credit_costos = abs(res['credit'])
                        total_ytd_costos = abs(res['ytd'])
                        total_balance_costos = abs(res['balance'])
                    elif res['account_category'] == 'expense':
                        total_init_gastos = abs(res['balanceinit'])
                        total_debit_gastos = abs(res['debit'])
                        total_credit_gastos = abs(res['credit'])
                        total_ytd_gastos = abs(res['ytd'])
                        total_balance_gastos = abs(res['balance'])
                    if aa_id[3].account_category == 'result':
                        total_init = abs(abs(total_init_ingreso) - abs(total_init_costos) - abs(total_init_gastos))
                        total_ytd = abs(abs(total_ytd_ingreso) - abs(total_ytd_costos) - abs(total_ytd_gastos))

                        res.update({
                            'balanceinit': total_init,
                            'debit': abs(abs(total_debit_ingreso) - abs(total_debit_costos) - abs(total_debit_gastos)),
                            'credit': abs(abs(total_credit_ingreso) - abs(total_credit_costos) - abs(total_credit_gastos)),
                            'ytd': total_ytd,
                            'balance': abs(total_init - total_ytd),
                            })
                    #archi_text += str(res['name']) + '\t' + str(res['code']) + '\t' + str(res['balanceinit']) + '\t' + str(res['debit']) + '\t' + str(res['credit']) + '\t' + str(res['balance']) + '\t' + str(res['ytd']) + '\t' + str(res['label']) + '\t' + str(res['total']) + '\t' + str(res['level']) + '\t' + str(res['account_category']) + '\n'
