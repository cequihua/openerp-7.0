...
class account_balance(report_sxw.rml_parse):
    ...
    def lines(self, form, level=0):
        ...
        sum_credit_cost_center = 0.0
        sum_debit_cost_center = 0.0

        sum_credit_mayor = 0.0
        sum_debit_mayor = 0.0

        total_account_mayor_debit_company = 0.0
        total_account_mayor_credit_company = 0.0
        ...
        total_debit_resultados_deudoras = 0.0

        total_credit_ingreso = 0.0
        total_credit_gastos = 0.0
        total_credit_costos = 0.0
        total_credit_resultados_deudoras = 0.0
        ...
        for aa_id in account_ids:
            ...
            if not form['display_account_level'] or aa_id[3].level <= form['display_account_level']:
                ...
                'label'     : aa_id[1],
                'total'     : aa_id[2],
                #'total'     : (aa_id[2] and not aa_id[1]) and False or True,
                ...
                if form['columns'] == 'qtr':
                    ...
                    for p_id in p:
                        ...
                        if form['inf_type'] == 'IS':
                            if cost_center_ids and aa_id[3].type == 'view' and aa_id[2] == True and aa_id[1] == False and aa_id[0] == account_parent_id:
                                aa_brw_end.debit = sum_debit_cost_center
                                aa_brw_end.credit = sum_credit_cost_center
                                account_parent_id = res['parent_id']
                                is_totaized = True

                                if aa_id[3].level == 1:
                                    aa_brw_end.debit = sum_debit_mayor
                                    aa_brw_end.credit = sum_credit_mayor
                                    sum_debit_mayor = 0.0
                                    sum_credit_mayor = 0.0

                            if cost_center_ids and aa_id[3].code == '0':
                                #pdb.set_trace()
                                aa_brw_end.debit = total_account_mayor_debit_company
                                aa_brw_end.credit = total_account_mayor_credit_company

                            if cost_center_ids and aa_id[3].type != 'view' and aa_id[2] == True and aa_id[1] == True:
                                ...
                                sum_debit_cost_center += total_debit
                                sum_credit_cost_center += total_credit
                                sum_credit_mayor += total_credit
                                sum_debit_mayor += total_debit
                                total_account_mayor_debit_company += total_debit
                                total_account_mayor_credit_company += total_credit
                                account_parent_id = res['parent_id']
                    ...
                    if cost_center_ids and aa_id[3].type != 'view' and aa_id[2] == True and aa_id[1] == True:
                        ...
                        i,d,c = map(z,[aa_brw_init.balance,aa_brw_end.debit,aa_brw_end.credit])
                        #i,d,c = map(z,[aa_brw_init.balance, total_debit, total_credit])
                        #print 'BALANCE ' + str(i)
                    if aa_id[3].code == '90000100' or aa_id[3].code == '90000000':
                        aa_brw_init.balance = total_init_ingreso - total_init_gastos - total_init_costos
                        aa_brw_end.debit = total_debit_ingreso - total_debit_gastos - total_debit_costos
                        aa_brw_end.credit = total_credit_ingreso - total_credit_gastos - total_credit_costos
                        i,d,c = map(z,[aa_brw_init.balance,aa_brw_end.debit,aa_brw_end.credit])  
                    else:
                        i,d,c = map(z,[aa_brw_init.balance,aa_brw_end.debit,aa_brw_end.credit])
                    b = z(i+d-c)
                    ...
                    #ACCOUNT INGRESOS
                    if aa_id[3].code == '40000000' and aa_id[1] == False and aa_id[2] == True:
                        total_init_ingreso += res['balanceinit']
                        total_debit_ingreso += res['debit']
                        total_credit_ingreso += res['credit']
                    #ACCOUNT COSTOS
                    if aa_id[3].code == '50000000' and aa_id[1] == False and aa_id[2] == True:
                        total_init_costos += res['balanceinit']
                        total_debit_costos += res['debit']
                        total_credit_costos += res['credit']
                    #ACCOUNT GASTOS
                    if aa_id[3].code == '60000000' and aa_id[1] == False and aa_id[2] == True:
                        total_init_gastos += res['balanceinit']
                        total_debit_gastos += res['debit']
                        total_credit_gastos += res['credit']
                ...
                if to_include and form['analytic_ledger'] and form['columns']=='four' and form['inf_type'] == 'BS' and res['type'] in ('other','liquidity','receivable','payable'):
                    ...
                    res['mayor'] = []
                #archi_text += str(res['name']) + str(res['code']) + str(res['balanceinit']) + '\t' + str(res['debit']) + '\t' + str(res['credit']) + '\t' + str(res['balance']) + '\t' + str(res['ytd']) + '\n'
                #archi_text += str(res['id']) + str(res['type']) + str(res['code']) + '\t' + str(res['name']) + '\t' + str(res['parent_id']) + '\t' + str(res['level']) + '\t' + str(res['label']) + str(res['total']) + '\n'
                if aa_id[3].code == '90000000' or aa_id[3].code == '90000100':
                    to_include = True
