...
class loan_partner_bank(osv.Model):
...
class account_loan(osv.Model):
    ...
    _columns = {
        ...
        'loan_id': fields.char('Prestamo Id', size=32, required=True),
        ...
        #'loan_type':fields.function(_loan_type_get, string='Loan Type', type="many2one",select=True, required=True),
        'loan_type':fields.many2one('account.loan.loantype','Loan Type', required=False),
        #'loan_period':fields.selection(_loan_period_get,'Loan Period',select=True, required=True),
        #'loan_period':fields.function(_loan_period_get, string='Loan Period', type="many2one", select=True, required=True),
        'loan_period':fields.many2one('loan.installment.period', 'Loan Period', required=True),
        'loan_amount': fields.float('Loan Amount',digits=(12,2), states={'draft':[('readonly',False)]}, required=True),
        'approve_amount': fields.float('Approve Amount',digits=(12,2),readonly=False),
        'process_fee': fields.float('Processing Fee',digits=(12,2), required=False),
        'total_installment': fields.integer('Total Installment', readonly=False),
        'interest_rate': fields.float('Interest Rate',digits=(10,1),readonly=False, required=True),
        'apply_date': fields.date('Fecha', states={'draft':[('readonly',False)]}),
        'approve_date': fields.date('Approve Date',readonly=False),
        'cheque_ids':fields.one2many('account.loan.bank.cheque','loan_id','Cheque Detail'),
        'state': fields.selection([
           ('draft','Cotizacion'),
           ('proof approval','Evaluacion'),
           ('approved','Autorizado'),
           ('cancel','Cancelado'),
        ...
        'voucher_id': fields.many2one('account.voucher', 'Voucher',readonly=False),
        'notes' : fields.text('Description'),
        'loan_acc': fields.property(
                'account.account',
                type='many2one',
                relation='account.account',
                string="Cuenta de Credito",
                method=True,
                view_load=True,
        ...
        'anal_acc': fields.property(
                ...
                method=True,
                view_load=True,
                help="This analytic account will be used", required=False,
        ),
        'cob_acc': fields.property(
                'account.account',
                type='many2one',
                relation='account.account',
        ...
        'loan_amt': fields.float('Loan Amount',digits=(12,2),required=False),
        ...
        'tasa_int_mensual': fields.float('Interes Mensual', digits=(10,1), required=True), 
        'tasa_int_periodo': fields.float('Interes por Periodo', digits=(10,1), required=True),  
        'cat': fields.float('CAT',readonly=False),     
    }
    _defaults = {
         ...
         'approve_date':lambda *a: time.strftime('%Y-%m-%d'),
    }
    ...
    def onchange_loan_period(self, cr, uid, ids, part):
        print part
        sear_list = self.pool.get('loan.installment.period').search(cr,uid,[('id','=',part)])
    ...
    def _simple_interest_get(self, cr, uid, ids, inter_rate):
        ...
        inter_sed=self.pool.get('account.loan.loantype').search(cr,uid,[('name','=',loan[0]['loan_type'][1])])
        if inter_sed:
        ...
        while i <= loan[0]['total_installment']:
            ...
            if loan[0]['return_type'] == 'cheque':
                ...
                #loan[0]['partner_id']
                #self.create_invoices(cr, uid, ids, ids[0], part_id, loan[0]['loan_amount'], loan[0]['interest_rate'], date_update, loan[0]['total_installment'], loan[0]['tasa_int_mensual'], loan[0]['tasa_int_periodo'])
                #self.pool.get('account.invoice').create(cr,uid,{'loan_id': ids[0], 'date_invoice' : date_update, 'partner_id' : part_id})          
            search_ids = cheque_cr.search(cr,uid,[('loan_id','in',ids)])
    ...
    def reject_loan(self, cr, uid, ids,context = {}):
        ...
        loan_payment_ids = loan_payment_cr.search(cr,uid,[('loan_id','=',ids[0])])
    ...
    def proof_approval(self, cr, uid, ids,context = {}):
        loan = self.pool.get('account.loan').read(cr,uid,ids)
        self.create_amortization_table(cr, uid, ids, loan[0]['loan_amount'], loan[0]['tasa_int_periodo'], loan[0]['total_installment'], ids[0])
        #self.pool.get('account.loan').write(cr,uid,ids,{'state':'apply'})
        return True
        # loan = self.pool.get('account.loan').read(cr,uid,ids)
        # loantype =  loan[0]['loan_type'][1]
        # print loantype
        # if loantype != '':
